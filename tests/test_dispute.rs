...
#[tokio::test]
async fn multi_tx_multi_dispute() {
    let data = "\
        type,client,tx,amount
        deposit,1,1,1.1
        dispute,1,1 
    ...
    let verify_account = Account::new(1)
        ...
        .set_total(Coin::new(33222, 4))
        .set_held(Coin::new(33222, 4));

    assert!(verify_account.check_amounts(accounts.get(&1).unwrap()));
}

#[tokio::test]
async fn multi_acc_dispute() {
    let data = "\
        type,client,tx,amount
        deposit,1,1,1.1
        deposit,2,2,2.2222 
